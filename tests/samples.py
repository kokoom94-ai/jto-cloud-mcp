...
def sample(kind):
    if kind=='onepage':return {
        ...
        'summary':['[제안] 개별여행객의 제주 체류 확대','예약 가능한 로컬 체험과 판매 채널 연계'],
        'sections':[
            {'heading':'추진방향','bullets':['[제안] 중화권 시장별 수요 조사 후 타깃 선정','[제안] 체류형 코스와 현지 판매 채널 연계']},
            {'heading':'실행계획','bullets':['[제안] 분기별 공동 프로모션과 콘텐츠 운영','[제안] 예약·이용 완료·직접 거래액 측정']},
    ...
    return {'project_name':'2027 중화권 마케팅','plan_title':'중화권 마케팅 '+('결과보고' if kind=='result_report' else '사업계획(안)'),
        ...
        'sections':{k:[{'text':'[미확인] 자료 입력 대기' if kind=='result_report' else '[제안] '+v+' 검토',
                       'details':['이 문서는 서식·생성 기능 검증용 예시이며 실제 사업 실적이 아닙니다.']}] for k,v in labels.items()},
        'budget':{'status':'unknown','account':'[미입력]','payment_method':'[미입력]',
                  'items':[{'category':'[미확인]','description':'[미확인] 예산 또는 집행내역 입력 대기','amount':0}]},
        'expected_effects':['[제안] 내부 자료 확인 후 계획을 보완한다.'],
